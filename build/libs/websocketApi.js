"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports["default"] = void 0;

var _validator = _interopRequireDefault(require("validator"));

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { "default": obj }; }

function _classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } }

function _defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); } }

function _createClass(Constructor, protoProps, staticProps) { if (protoProps) _defineProperties(Constructor.prototype, protoProps); if (staticProps) _defineProperties(Constructor, staticProps); return Constructor; }

var WebSocketAPI =
/*#__PURE__*/
function () {
  function WebSocketAPI() {
    _classCallCheck(this, WebSocketAPI);

    this._list = [];
  }

  _createClass(WebSocketAPI, [{
    key: "on",
    value: function on(event, callback) {
      this._list.push({
        name: event,
        callback: callback
      });
    }
  }, {
    key: "parseEvent",
    value: function parseEvent(name, data) {
      if (typeof data == 'String' && _validator["default"].isJSON(data)) data = JSON.parse(data);

      var searched = this._list.find(function (element) {
        return element.name === name;
      });

      if (searched == null) return false;
      searched.callback(searched.data);
    }
  }]);

  return WebSocketAPI;
}();

exports["default"] = WebSocketAPI;